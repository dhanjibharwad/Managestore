...
const StorageLocationsPage = ({ addModal = false, setAddModal, searchQuery = '' }: StorageLocationsPageProps = {}) => {
  ...
  const [editModal, setEditModal] = useState<{show: boolean, location: StorageLocation | null}>({show: false, location: null});
  const [deleteModal, setDeleteModal] = useState<{show: boolean, location: StorageLocation | null}>({show: false, location: null});
  ...
  const handleEdit = async () => {
    if (!editModal.location || !formName.trim()) return;
    setSubmitting(true);
    try {
      const response = await fetch('/api/admin/storage-locations', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: editModal.location.id, name: formName.trim() })
      });
      if (response.ok) {
        setEditModal({show: false, location: null});
        setFormName('');
        fetchStorageLocations();
      }
    } finally {
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`/api/admin/storage-locations?id=${deleteModal.location.id}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-700 uppercase">Storage Location</th>
            ...
            {loading ? (
              ...
              filteredStorageLocations.map((location, index) => (
                <tr key={location.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-sm text-blue-600">{formatDate(location.created_at)}</td>
                  <td className="px-6 py-4 text-center relative">
                    ...
                    {openDropdown === index && (
                      <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                        <button onClick={() => { setEditModal({show: true, location}); setFormName(location.name); setOpenDropdown(null); }} className="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                          <Edit2 className="w-4 h-4" />Edit
                        </button>
                        <button onClick={() => { setDeleteModal({show: true, location}); setOpenDropdown(null); }} className="w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-full max-w-md p-6">
            <h3 className="text-lg font-semibold mb-4">Add Storage Location</h3>
            <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="Storage Location Name" className="w-full px-3 py-2 border border-gray-300 rounded-md mb-4" />
      ...
      {editModal.show && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-full max-w-md p-6">
            <h3 className="text-lg font-semibold mb-4">Edit Storage Location</h3>
            ...
            <div className="flex gap-3">
              <button onClick={() => setEditModal({show: false, location: null})} className="flex-1 px-4 py-2 border border-gray-300 rounded-md">Cancel</button>
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg w-full max-w-md p-6">
            <h3 className="text-lg font-semibold mb-4">Delete Storage Location</h3>
            <p className="mb-4">Are you sure you want to delete <strong>{deleteModal.location?.name}</strong>?</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteModal({show: false, location: null})} className="flex-1 px-4 py-2 border border-gray-300 rounded-md">Cancel</button>
