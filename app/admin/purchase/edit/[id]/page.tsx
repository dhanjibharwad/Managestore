...
export default function EditPurchasePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const getParams = async () => {
      ...
      if (companyId) {
        fetchPurchase(resolvedParams.id);
      }
      fetchSuppliers();
    };
    getParams();
  ...
  const fetchPurchase = async (id: string) => {
    ...
    try {
      ...
      setPartyInvoiceNumber(purchase.party_invoice_number || '');
      setPurchaseDate(purchase.purchase_date ? purchase.purchase_date.split('T')[0] : '');
  ...
  const handleSavePart = () => {
    if (!modalPartName && !modalDescription) {
      showToast('Please enter part name or description', 'warning');
      return;
    }
    
    if (!modalPrice || parseFloat(modalPrice) <= 0) {
    ...
    try {
      const response = await fetch(`/api/admin/purchases/${purchaseId}`, {
        ...
        body: JSON.stringify({
          purchaseNumber,
          supplierName,
          partyInvoiceNumber: partyInvoiceNumber || null,
          purchaseDate,
      ...
      showToast('An error occurred while updating the purchase', 'error');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="bg-gray-50">
      ...
      <div className="mx-auto bg-white rounded-lg shadow-sm">
        ...
        <div className="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-semibold text-gray-900">Edit Purchase : {purchaseNumber}</h1>
