            ...
            <div className="grid grid-cols-3 gap-6">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {Array.isArray(customers) && customers.map((cust, index) => (
                      <option key={`customer-${cust.customer_id || cust.id}-${index}`} value={cust.id}>
              ...
              <div>
                ...
                >
                  <option value="">Select customer</option>
                  {Array.isArray(customers) && customers.map((cust, index) => (
                    <option key={`referred-${cust.customer_id || cust.id}-${index}`} value={cust.id}>
                      {cust.customer_name} - {cust.customer_id} ({cust.mobile_number})
                    </option>
                  ))}
                </select>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const value = e.target.value;
                      // Allow only numbers and decimal point, but don't restrict based on parseFloat
                      if (/^\d*\.?\d*$/.test(value)) {
                        setQuantity(value);
                      }
                    }}
