...
export async function POST(req: NextRequest) {
  try {
    ...
    // Validation
    if (!companyId || !customerName || !assignee || !amcType || !contractStartDate || !contractEndDate) {
      return NextResponse.json({ error: 'Required fields missing' }, { status: 400 });
    }

    const query = `
      INSERT INTO amc_contracts (
        company_id, customer_name, assignee, amc_type, contract_start_date, contract_end_date,
