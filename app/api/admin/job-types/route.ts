...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await pool.query(
      'SELECT id, name, created_at, updated_at FROM job_types WHERE company_id = $1 ORDER BY name ASC',
      [session.company.id]
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO job_types (company_id, name) VALUES ($1, $2) RETURNING id, name',
    ...
    if (error.code === '23505') {
      return NextResponse.json({ error: 'Job type already exists' }, { status: 409 });
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (!id || !name || !name.trim()) {
      return NextResponse.json({ error: 'Job type ID and name are required' }, { status: 400 });
    }

    const result = await pool.query(
      'UPDATE job_types SET name = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND company_id = $3 RETURNING id, name',
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const result = await pool.query(
      'DELETE FROM job_types WHERE id = $1 AND company_id = $2 RETURNING id',
